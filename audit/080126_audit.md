...
| API Route | Category | Instrumentation Status | Notes |
|-----------|----------|------------------------|-------|
| `/api/auth/*` | Authentication | ✅ Instrumented | Via Supabase auth hooks |
| `/api/admin/*` | Admin | ✅ Instrumented | Admin activity log table |
| `/api/user/apps` | User Data | ⚠️ Needs Instrumentation | User app changes not logged |
| `/api/professionals` | Public | 🟡 Optional | Read-only, low priority |
| `/api/gigs` | Public | 🟡 Optional | Read-only, low priority |
| `/api/musicians` | Public | 🟡 Optional | Read-only, low priority |
| `/api/contact-requests` | User Action | ⚠️ Needs Instrumentation | Privacy-sensitive requests |
| `/api/feedback` | User Action | ✅ Instrumented | Stored in feedback table |
| `/api/onboarding/*` | User Data | ⚠️ Needs Instrumentation | Profile setup not logged |
| `/api/reviews` | User Action | ✅ Instrumented | Stored in reviews table |
