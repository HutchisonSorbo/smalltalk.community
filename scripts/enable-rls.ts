...
async function main() {
    try {
        ...
        EXISTS (SELECT 1 FROM band_members WHERE band_id = id AND user_id = auth.uid()::text AND role = 'admin')
            ...
            // Allow users to create notifications for themselves (e.g. reminders)
            `DROP POLICY IF EXISTS "Users can insert their own notifications" ON notifications`,
            `CREATE POLICY "Users can insert their own notifications" ON notifications FOR INSERT WITH CHECK (auth.uid()::text = user_id)`,
            // Note: Server actions (service role) bypass RLS, so no specific policy needed for cross-user notifications generated by system.
