...
function MFASecurityCard({ factors }: { factors: any[] }) {
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
...
export default async function SecuritySettingsPage() {
    ...
    try {
        ...
        if (result.error) {
            console.error("[SecuritySettingsPage] MFA error:", result.error);
        } else {
            factors = result.factors || [];
        }
    } catch (err) {
        console.error("[SecuritySettingsPage] Unexpected error:", err);
    }
    ...
    return (
        <div className="space-y-6 max-w-full">
            ...
            <div className="grid gap-6">
                <PasswordCard />
                <MFASecurityCard factors={factors} />
                <SessionsCard />
            </div>
        </div>
    );
